...
#         raise NotImplementedError()
    ...
    def to_linear_combinaison_pauli_string(self,
        ...
        h_matrix = self.integrals
        ap_list = creation_operators
        am_list = annihilation_operators
        
        k = 0    # index for new_coef and new_pauli_string, ranges from 0 to (2*n_orbs)**2
        for i in range(n_orbs):
            for j in range(n_orbs):
                LCPS = float(h_matrix[i,j])*ap_list[i]*am_list[j]  #lcsp must be multilplied with a float,
                                                                    #h_matrix[i,j] is a numpy-float
                coeff_list = LCPS.coefs  
                pauli_string_list = LCPS.pauli_strings
                for l, cf in enumerate(coeff_list):
                    new_coefs[k] = cf
                    new_pauli_strings[k] = pauli_string_list[l]
                    k+=1
...
#         raise NotImplementedError()
    ...
    def to_linear_combinaison_pauli_string(self,
        ...
        n = 0    # index for new_coef and new_pauli_string, ranges from 0 to (2*n_orbs)**2
        for i in range(n_orbs):
            for j in range(n_orbs):
                for k in range(n_orbs):
                    for l in range(n_orbs):
                        LCPS = 0.5*float(h_matrix[i,j,k,l])*ap_list[i]*ap_list[j]*am_list[k]*am_list[l]
                        
                        coeff_list = LCPS.coefs
                        pauli_string_list = LCPS.pauli_strings
                        for _, cf in enumerate(coeff_list):
                            new_coefs[n] = cf
                            new_pauli_strings[n] = pauli_string_list[_]
                            n+=1
...
class MolecularFermionicHamiltonian(FermionicHamiltonian):
    ...
    def to_linear_combinaison_pauli_string(self,
        ...
        # YOUR CODE HERE
        # TO COMPLETE (after lecture on mapping)
        H2 = self.two_body.to_linear_combinaison_pauli_string(creation_operators, annihilation_operators)
        H1 = self.one_body.to_linear_combinaison_pauli_string(creation_operators, annihilation_operators)
